...
<<<<<<< HEAD
	send_text(recipient_id, message) {
		...
		return axios.post(`${this.get_def_api_url() + this.get_def_endpoint()}?access_token=${this.get_access_token()}`, data, {
			headers: {
			...
			console.log("Failed to send message: " + JSON.stringify(error.response.data.error));
		});
	}
...
<<<<<<< HEAD
  axios.post(this.get_def_api_url() + this.get_def_endpoint(), data, {
				 params: {
			...
			console.log("Failed to send message : " + JSON.stringify(e.response));
		})
}*/
=======
 axios(this.get_def_api_url() + this.get_def_endpoint(), data, {method:"POST",
      params: {
        access_token: this.get_access_token()
      }
    }).then((response)=>{
    return response.data;
	 ...
	 console.log("Failed to send message : " + e.response);
 })
}
>>>>>>> 239a206bbf7bdc660aeb816be65747d4d7f321de
