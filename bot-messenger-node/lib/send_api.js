...
class SendApi {
  ...
  send_text(message, recipient_id, messaging_type = 'RESPONSE', notification_type = 'REGULAR', kwargs=null) {
    ...
    if (messaging_type === 'MESSAGE_TAG') {
      assert(kwargs.tag in ['ACCOUNT_UPDATE', 'CONFIRMED_EVENT_UPDATE', 'CUSTOMER_FEEDBACK', 'HUMAN_AGENT', 'POST_PURCHASE_UPDATE'], "value of param messagin_type must be 'ACCOUNT_UPDATE','CONFIRMED_EVENT_UPDATE','CUSTOMER_FEEDBACK','HUMAN_AGENT' or 'POST_PURCHASE_UPDATE'");
